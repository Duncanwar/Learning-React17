...
export function jwtVerify(accessToken: string, cb: any): void {
  //Verifying our JWT token using the accessToken and secretKey
  jwt.verify(
    accessToken,
    secretKey,
...
export async function getUserData(accessToken: string): Promise<any> {
  ...
  // This will get user data or false (if the user is not connected)
...
export const createToken = async (user: IUser): Promise<string[]> => {
  ...
  // We sign our JWT token ans we save the data as Base64
